...
http.createServer((req,res)=>{
 ...
 if(pathname==='/health')return json(res,200,{ok:true,service:'smart-adhkar-web',version:'2.4.0',audio:false,contentPolicy:'trusted-sources',ui:'unified-v24'});
 ...
 fs.stat(file,(err,stat)=>{if(err||!stat.isFile()){res.writeHead(404,{'content-type':'text/plain; charset=utf-8'});return res.end('Not found')}
  ...
  if(pathname==='/index.html')return fs.readFile(file,'utf8',(e,html)=>{if(e)return res.end('');const css='<link rel="stylesheet" href="/v24.css">';const js='<script src="/v24.js" defer></script>';if(!html.includes('/v24.css'))html=html.replace('</head>',css+'</head>');if(!html.includes('/v24.js'))html=html.replace('</body>',js+'</body>');res.end(html)});
  fs.createReadStream(file).pipe(res);
 });
}).listen(process.env.PORT||3000,'0.0.0.0',()=>console.log('Smart Adhkar web v2.4.0 unified-ui running'));
