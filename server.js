...
http.createServer((req,res)=>{
  ...
  if(pathname === '/health'){res.writeHead(200,{'content-type':'application/json'});return res.end(JSON.stringify({ok:true,service:'smart-adhkar-web',version:'1.2.0'}));}
  ...
  fs.stat(file,(err,stat)=>{
    ...
    if(pathname==='/index.html'){
      fs.readFile(file,'utf8',(e,html)=>{
        if(e)return res.end('');
        const tag='<script src="/v12.js"></script>';
        if(!html.includes('/v12.js')) html=html.replace('</body>',tag+'</body>');
        res.end(html);
      });
      return;
    }
...
}).listen(process.env.PORT||3000,'0.0.0.0',()=>console.log('Smart Adhkar web v1.2.0 running'));
