...
let tasbeehDhikr=localStorage.tasbeehDhikr||'سبحان الله';
...
function renderCard(d){const f=favs().has(d.id);return `<article class="dhikr-card"><div class="card-top"><span class="pill">${d.category}</span><button class="heart ${f?'on':''}" data-fav="${d.id}" aria-label="المفضلة">${f?'♥':'♡'}</button></div><h3>${d.title}</h3><p class="text">${d.text}</p><div class="meta"><span>التكرار: ${d.target}</span>${d.timeHint?`<span>• ${d.timeHint}</span>`:''}</div>${d.sourceName?`<div class="source-inline"><span>✓ موثّق</span><small>${d.sourceName}</small></div>`:''}<div class="card-actions"><a class="secondary" href="${d.sourceUrl||'#'}" target="_blank" rel="noopener">↗ المصدر</a></div></article>`}
function wireCards(){$$('[data-fav]').forEach(b=>b.onclick=()=>{const s=favs();s.has(b.dataset.fav)?s.delete(b.dataset.fav):s.add(b.dataset.fav);saveFavs(s);renderAll();renderFavorites();toast('تم تحديث المفضلة')})}
...
function updateSelectedDhikr(){const select=$('#tasbeehDhikr');const custom=$('#customDhikr');const wrap=$('#customDhikrWrap');if(!select)return;const isCustom=select.value==='custom';wrap.hidden=!isCustom;if(isCustom){tasbeehDhikr=(custom.value||localStorage.customTasbeehDhikr||'ذكر مخصص').trim()}else tasbeehDhikr=select.value;localStorage.tasbeehDhikr=tasbeehDhikr;if(isCustom)localStorage.customTasbeehDhikr=custom.value;$('#selectedDhikr').textContent=tasbeehDhikr||'ذكر مخصص'}
function initTasbeehDhikr(){const select=$('#tasbeehDhikr');const custom=$('#customDhikr');if(!select||!custom)return;const preset=[...select.options].some(o=>o.value===tasbeehDhikr);if(preset)select.value=tasbeehDhikr;else{select.value='custom';custom.value=localStorage.customTasbeehDhikr||tasbeehDhikr}updateSelectedDhikr()}
function renderTasbeeh(){tasbeeh=Math.min(tasbeeh,tasbeehGoal);$('#tasbeehCount').textContent=tasbeeh;$('#tasbeehRemaining').textContent=`المتبقي ${Math.max(0,tasbeehGoal-tasbeeh)}`;$('#tasbeehBar').style.width=`${Math.min(100,tasbeeh/tasbeehGoal*100)}%`;localStorage.tasbeeh=String(tasbeeh);localStorage.tasbeehGoal=String(tasbeehGoal);localStorage.tasbeehDhikr=tasbeehDhikr}
...
function streakCount(){const history=getJSON('completionHistory',{});let streak=0;const d=new Date();for(let i=0;i<365;i++){const key=localDateKey(d);if((history[key]||[]).length>0){streak++;d.setDate(d.getDate()-1)}else{if(i===0){d.setDate(d.getDate()-1);continue}break}}return streak}
function renderWeek(){const history=getJSON('completionHistory',{});const days=[];const fmt=new Intl.DateTimeFormat('ar-SA',{weekday:'short'});for(let i=6;i>=0;i--){const d=new Date();d.setDate(d.getDate()-i);const key=localDateKey(d);const count=(history[key]||[]).length;days.push({label:fmt.format(d),count})}$('#weekBars').innerHTML=days.map(x=>`<div><i style="height:${Math.max(8,x.count/3*100)}%"></i><strong>${x.count}</strong><span>${x.label}</span></div>`).join('')}
...
function backupPayload(){const keys=['favorites','theme','tasbeeh','tasbeehGoal','tasbeehDhikr','customTasbeehDhikr','arabicVoiceName','arabicVoiceURI','voiceRate','tasbeehLifetime','dhikrTaps','routineCompletionCount','completionHistory','routine-morning','routine-evening','routine-prayer','completed-morning','completed-evening','completed-prayer'];const data={version:3,app:'Smart Adhkar Web',exportedAt:new Date().toISOString(),storage:{}};keys.forEach(k=>{if(localStorage.getItem(k)!==null)data.storage[k]=localStorage.getItem(k)});return data}
...
async function init(){const parts=await Promise.all([1,2,3,4].map(i=>fetch(`/adhkar-${i}.json`).then(r=>r.json())));adhkar=parts.flat();initCategories();renderAll();themeInit();initTasbeehDhikr();renderTasbeeh();loadRoutine('morning');renderStats();setupInstall();if('serviceWorker'in navigator)navigator.serviceWorker.register('/sw.js').catch(()=>{})}
...
$('#routineSpeak').onclick=()=>speakDhikr(routineItems[routineIdx]);
$$('[data-goal]').forEach(b=>b.onclick=()=>setGoal(Number(b.dataset.goal)));
$('#customGoal').onchange=e=>setGoal(Number(e.target.value));
$('#tasbeehDhikr').onchange=()=>{tasbeeh=0;updateSelectedDhikr();renderTasbeeh()};
$('#customDhikr').oninput=()=>{updateSelectedDhikr();renderTasbeeh()};
$('#tasbeehSpeak').onclick=()=>speakText(tasbeehDhikr);
$('#tapCounter').onclick=()=>{if(tasbeeh<tasbeehGoal){tasbeeh++;incrementStat('tasbeehLifetime',1);if(navigator.vibrate)navigator.vibrate(tasbeeh===tasbeehGoal?[70,40,110]:12);renderTasbeeh();renderStats();if(tasbeeh===tasbeehGoal)toast(`تم هدف ${tasbeehDhikr} بحمد الله`)}};
$('#tasbeehReset').onclick=()=>{tasbeeh=0;renderTasbeeh()};
$('#tasbeehMinus').onclick=()=>{tasbeeh=Math.max(0,tasbeeh-1);renderTasbeeh()};
$('#voiceSelect').onchange=e=>{localStorage.arabicVoiceName=e.target.value;const v=preferredArabicVoice();$('#voiceStatus').textContent=v?`الصوت المختار: ${v.name} (${v.lang})`:'سيتم اختيار أفضل صوت عربي متاح تلقائيًا.'};
